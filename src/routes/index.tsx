...
function Home() {
  ...
  const events = useMemo(
    ...
    [today, feed, when, categories, cities, query, savedOnly, savedIds],
  );

  const featured = useMemo(
    () => featuredEvent(events, nowMs),
    [events, nowMs],
  );
  const grouped = useMemo(() => groupByDay(events), [events]);
  const cityList = useMemo(() => cityChips(today, feed), [today, feed]);
  const selected = selectedId ? (EVENTS_BY_ID[selectedId] ?? null) : null;

  return (
    <main className="min-h-dvh bg-bg pt-safe text-fg">
      <div className="mx-auto max-w-6xl px-4 py-8 pb-safe md:px-8 md:py-10">
        <div className="rise-in">
          <AppHeader today={today} feed={feed} />
        </div>
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,18rem)_minmax(0,1fr)] lg:items-start">
          ...
          <section className="relative min-w-0">
            {featured ? (
              <div className="mb-10">
                <Featured event={featured} nowMs={nowMs} today={today} onOpen={setSelectedId} />
              </div>
            ) : null}
            ...
            {events.length === 0 ? (
              ...
              <div className="flex flex-col gap-10">
                {grouped.map((day) => (
                  <section key={day.ymd} className="day-block">
                    ...
                    <div>
                      {day.events.map((event) => (
                        <EventCard
                          key={event.id}
                          event={event}
                          nowMs={event.ymd === today ? nowMs : event.startMs}
                          today={today}
